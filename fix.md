# RL2 Assessment Findings and Recommended Fixes

**Date:** 2025-01-04
**Target:** RL2 Core Ontology, Roadmap, and Coverage Documentation

## 1. Roadmap vs. Ontology Synchronization

**Finding:** There is a significant discrepancy between the phased, modular approach described in `RL2_Roadmap.md` and the actual state of the `rl2.ttl` ontology.
*   **Roadmap:** Describes a "Core" profile (Phase 0) followed by "Hohfeldian" (Phase 1), "Events" (Phase 2), "Promises" (Phase 2), and "Generations" (Phase 3) profiles.
*   **Ontology:** `rl2.ttl` (v0.3) currently contains **all** these features in a single monolithic file. It includes Hohfeldian classes (`Claim`, `Power`, etc.), Event constructs (`Event`, `EventConstraint`), Promises, and Generations.

**Impact:** The roadmap presents these as future or separate extensions, but they are already implemented in the main artifact. This creates confusion about the maturity and modularity of the project.

**Recommendation:**
*   **Option A (Reflect Reality):** Update `RL2_Roadmap.md` to reflect that v0.3 is already a feature-complete alpha containing these "future" features, and subsequent phases are for *refinement* and *profile separation*, not initial introduction.
*   **Option B (Enforce Modularity):** Split `rl2.ttl` into `rl2-core.ttl`, `rl2-hohfeldian.ttl`, `rl2-events.ttl`, etc., to match the roadmap's architecture.

## 2. ODRL Superset Claim Assessment

**Finding:** The claim in `RL2_ODRL_Coverage.md` that "RL2 is a strict superset of ODRL" is technically inaccurate regarding structural compatibility, though semantically strong.
*   **Inheritance (`odrl:inheritFrom`):** RL2 explicitly excludes runtime inheritance, requiring a pre-processing/flattening step. While semantically equivalent results can be achieved, it is not a syntactic superset (cannot consume valid ODRL with inheritance directly).
*   **Conflict Strategies (`odrl:conflict`):** ODRL allows policies to specify their conflict resolution strategy (e.g., `perm:perm`, `prohibit:perm`). `rl2.ttl` lacks an equivalent property to define the conflict strategy per policy, relying instead on implicit or implementation-defined logic.
*   **Remedies (`odrl:remedy`):** `rl2.ttl` lacks a direct `remedy` property for Duties, though it can likely be modeled via other means.
*   **Permission-bound duties (CRITICAL):** ODRL duties can have actions different from the permitted action (e.g., permit `display` if duty `pay` is fulfilled). RL2 semantics currently only activate duties whose `action` matches the request, so duties with different actions never apply. This breaks behavioral equivalence with ODRL and undermines the superset claim.
    ...
    **Example Usage:**
    ...
    ```
*   **Policy types / Requests:** ODRL defines `odrl:Request` as a policy class. `rl2.ttl` still lacks a policy-level Request; `rl2p.ttl` introduces a runtime `rl2p:Request` (evaluation artifact, not a policy container). This narrows but does not eliminate the gap—ODRL Request policies carrying rules still require a mapping strategy or a policy-class counterpart.

**Impact:** "Strict superset" implies full backward compatibility. Valid ODRL policies using inheritance or specific conflict strategies may not be valid or functionally equivalent RL2 policies without transformation.

**Recommendation:**
*   Soften the claim to "Semantic Superset" or "Expressive Superset".
*   Explicitly document the transformation required for `inheritFrom`.
*   Add a `rl2:conflictStrategy` property to `rl2:Policy` to support ODRL's conflict resolution selection.
*   Add a way to bind duties to privileges regardless of differing actions (e.g., linking duties to clauses instead of request-matching) to preserve ODRL's duty semantics.
*   Add a `rl2:Request` policy subclass (or extend `rl2p:Request` with policy semantics) plus a documented mapping for `odrl:Request`.

## 3. Deontic Logic Implementation Gaps

**Finding:** The `@deontic_logic_analysis.md` strongly recommends a specific "Recommended Approach" (Section 12) which is not fully realized in `rl2.ttl`.
*   **Missing Priority:** The analysis calls for "Explicit priority attributes" (`rl2:priority`) to support Defeasible Logic and "Lex Superior". This property is **missing** from `rl2.ttl`.
*   **Missing Specificity/Override:** The analysis discusses "specificity-based override" as a core feature. The ontology lacks the vocabulary to support this (e.g., `rl2:overrides`, `rl2:priority`).
*   **Missing Conflict Strategy Selector:** As noted above, the "Configurable priority strategy" mentioned in the analysis is not supported in the ontology.

**Impact:** The ontology cannot currently support the advanced conflict resolution (Defeasible Logic) that the analysis deems "Essential".

**Recommendation:**
*   Add `rl2:priority` (xsd:integer) to `rl2:Norm`.
*   Add `rl2:conflictStrategy` to `rl2:Policy`.
*   Consider adding `rl2:overrides` (ObjectProperty -> Norm) for explicit exclusions/exceptions as suggested by LegalRuleML analysis.

## 4. Missing Action Vocabulary

**Finding:** `RL2_Roadmap.md` claims "RL2-Core defines seven interoperable actions" (`rl2:read`, `rl2:write`, etc.). However, `rl2.ttl` defines the `rl2:Action` class but contains **no individual action instances**.

**Impact:** The core profile is incomplete according to its own definition.

**Recommendation:**
*   Add the seven core action instances to `rl2.ttl` or create the referenced `rl2-actions-minimal.ttl` file.

## 5. Test Suite Gap

**Finding:** `RL2_Roadmap.md` (lines 483–560) describes a 50+ test core suite and a `tests/` directory layout, but the repository currently has no `tests/` directory or test files.

**Impact:** Documentation overstates implementation readiness and conformance evidence.

**Recommendation:** Add the referenced test assets or adjust the roadmap to mark the suite as TODO with owners and milestones.
