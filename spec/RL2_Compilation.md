...
|---|---|---|
| `Request` | `RL2_Model.md` §3 | `parameters` as a canonical-JSON object keyed by name; `Numeric` parameter values as strings; `id` (optional) as an IRI or plain string, absent when the caller supplies none — a caller-supplied per-grant/decision identifier, never minted by `Eval`. |
| `WorldSnapshot` | `RL2_Model.md` §4 | `facts` and `evidence` as canonical-JSON arrays in a fixed sort order (by `id`), so two structurally equal snapshots serialize identically. Each `evidence` element's `dischargeOf` (optional) serializes as the occurrence-identifier tuple it names — `(sourceIdentity, Request.id)` or `(sourceIdentity, agent, asset)` — under the same canonical-JSON conventions, or is omitted when the assembler cannot correlate the evidence to a specific grant. |
...
| `EvaluationResult` | `RL2_Model.md` §6 | Replay-anchoring fields added at the interchange layer, listed below. These fields are additive to the abstract `EvaluationResult` value; they do not change its meaning, only what a caller retains alongside it. `dutyStatuses` entries keyed by a bound occurrence serialize their `occurrenceId` or `BoundIdentity` (`RL2_Model.md` §7.2) as an ordinary canonical-JSON map key alongside Duty-identity keys — the two key kinds are disjoint by construction (`RL2_Semantics.md`'s `boundOccurrenceStatuses`), so a caller re-querying a specific grant's Duty status later replays the same key rather than re-deriving the binding from the original Request and snapshot. |
