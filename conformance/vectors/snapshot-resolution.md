...
| F2 | Two valid facts for one single-valued key have distinct semantic values. | `Err(Conflict(key))` |
| F3 | No valid fact exists for the requested key and evaluation time. | `Err(Missing(key))` |
| F4 | A fact has an unsupported type or inadmissible attribution. | `Err(Invalid(key))` |
...
| E3 | A matching action record has inadmissible attribution. | `Err(Invalid(selector))` |
